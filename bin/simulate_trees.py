...
def main(params):
    ...
    if params.sd_fixed is not None:
        sd_range = [params.sd_fixed]
    else:
        ...
        if i == 0:
            ...
            if selected_tree['Colless'] <= last_colless:
                ...
                if valid_trees.empty:
                    print(f"WARN: No valid tree found for sd={sd} that is greater than the last selected tree's colless value.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sd_fixed', type=float, required=False, default=None,
                        help="Fixed SD value.")
